...
export default function ActualCost(props) {
  ...
  let actualSum = actualCosts.reduce(function (acc, obj) {
    return acc + obj.amount;
  }, 0);
  ...
  return (
    <div className="ActualCost">
      <div className="secondary-nav">
        ...
        <Link to="/budget/costs" className="selected-second-nav">
          Actual
        </Link>
                ...
                {actualCosts.map((c) => (
                  <tr key={c.id}>
                    <td
                      ...
                      onClick={(e) => getOneCostActual(c.id)}
                    >
                      {c.text}
                    </td>
                    <td
                    ...
                    <td
                      ...
                      className="cursor-pointer amount-td"
                      onClick={(e) => getOneCostActual(c.id)}
                    >
                      {props.allIncome.find((i) => i.id === c.income_id)
                        ? props.allIncome.find((i) => i.id === c.income_id).text
                        : ""}
                    </td>
                    <td
                      className="cursor-pointer"
                    ...
                    >
                      <button
                        type="submit"
                        onClick={(e) => handleClick(c.id, c.income_id)}
                      >
                        x
                      </button>
                    </td>
